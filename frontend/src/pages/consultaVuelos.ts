...
import { buscarVuelos, getVueloById } from '../services/vueloService';

// Variables para guardar los catálogos y evitar cargarlos múltiples veces
...
export async function renderConsultaVuelos(container: HTMLDivElement) {
  ...
  const setActiveTab = (tabName: 'buscar' | 'info') => {
    if (tabName === 'buscar') {
      ...
      tabBuscar.classList.remove('border-transparent', 'text-gray-500', 'hover:text-gray-700', 'hover:border-gray-300');
      tabInfo.classList.add('border-transparent', 'text-gray-500', 'hover:text-gray-700', 'hover:border-gray-300');
      ...
      tabInfo.classList.remove('border-transparent', 'text-gray-500', 'hover:text-gray-700', 'hover:border-gray-300');
      tabBuscar.classList.add('border-transparent', 'text-gray-500', 'hover:text-gray-700', 'hover:border-gray-300');
    ...
    container.querySelector<HTMLDivElement>('#consulta-resultados')!.innerHTML = '';
  };

  tabBuscar.addEventListener('click', () => {
    setActiveTab('buscar');
    renderBuscarForm(contentBuscar);
  });
  
  tabInfo.addEventListener('click', () => {
    setActiveTab('info');
    renderInfoForm(contentInfo);
  });
...
/**
 * Renderiza el formulario de la Pestaña "Buscar Vuelos"
 */
function renderBuscarForm(container: HTMLDivElement) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const errorEl = (container.getRootNode() as Document).querySelector<HTMLParagraphElement>('#buscar-error')!;
    ...
    const resultadosDiv = (container.getRootNode() as Document).querySelector<HTMLDivElement>('#consulta-resultados')!;
...
/**
 * Renderiza el formulario de la Pestaña "Información de Vuelo"
 */
/*function renderInfoForm(container: HTMLDivElement) {
    container.innerHTML = `
    <form id="info-form" class="p-6 bg-gray-50 rounded-lg shadow-inner">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="md:col-span-2">
          <label for="vuelo_id" class="block text-sm font-medium text-gray-700">ID de Vuelo:</label>
          <input type="text" id="vuelo_id" placeholder="Ingrese el ID del vuelo" required 
                 class="w-full mt-1 px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
        <div class="self-end">
          <button type="submit" 
    ...
    form.addEventListener('submit', async (e) => {
        e.preventDefault();
        const vueloId = (container.querySelector<HTMLInputElement>('#vuelo_id')!).value;
        const resultadosDiv = (container.getRootNode() as Document).querySelector<HTMLDivElement>('#consulta-resultados')!;
        resultadosDiv.innerHTML = `<p class="text-center text-gray-600">Consultando información...</p>`;

        const result = await getVueloById(vueloId);
...
// Cambiar el formulario de "Información de Vuelo" para que reciba 'numero_vuelo'
function renderInfoForm(container: HTMLDivElement) {
    container.innerHTML = `
    <form id="info-form" class="p-6 bg-gray-50 rounded-lg shadow-inner">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="md:col-span-2">
          ...
          <input type="text" id="numero_vuelo" placeholder="Ingrese el número del vuelo" required 
                 class="w-full mt-1 px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
        <div class="self-end">
          <button type="submit" 
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const result = await getVueloById(numeroVuelo);  // Cambiar a getVueloById con numero_vuelo
...
function renderResultadosVuelos(container: HTMLDivElement, vuelos: any[]) {
  ...
  container.innerHTML = vuelos.map(vuelo => `
    <div class="bg-white shadow-md rounded-lg p-4 mb-4 flex flex-col md:flex-row justify-between md:items-center">
      ...
      <div class="text-left md:text-right">
        ...
        <button class="mt-2 bg-gray-600 text-white py-2 px-4 rounded-md font-semibold hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition duration-200" 
                data-vuelo-id="${vuelo.id_vuelo}">
          Reservar
        </button>
      </div>
    </div>
  ...
  // (Aquí añadirías los event listeners para los botones "Reservar")
}
...
function renderResultadoInfoVuelo(container: HTMLDivElement, vuelo: any) {
    ...
    } else if (horaSalida.getTime() - ahora.getTime() < 30 * 60 * 1000) { // 30 mins
